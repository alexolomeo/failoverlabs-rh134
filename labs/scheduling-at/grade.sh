...
case "$LOG_LEVEL" in
    none|info)
        ...
        sudo ansible-playbook -i /usr/local/rh134/labs/server.lab /usr/local/rh134/labs/$LAB_NAME.yml -e "failover_mode='grade'" > /tmp/$LAB_NAME.log 2>&1
        ;;
    basic)
        # BASIC: Se muestra directamente en pantalla (sin redirigir a archivo)
        sudo ansible-playbook -i /usr/local/rh134/labs/server.lab /usr/local/rh134/labs/$LAB_NAME.yml -e "failover_mode='grade'"
        ;;
    debug)
        # DEBUG: Modo muy verboso (-vvvv) en pantalla
        sudo ansible-playbook -i /usr/local/rh134/labs/server.lab /usr/local/rh134/labs/$LAB_NAME.yml -e "failover_mode='grade'" -vvvv
        ;;
esac
...
sleep 2
print_line "· Generate Ponderation on servera" "CHECKING"
sleep 2
